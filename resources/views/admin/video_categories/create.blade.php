...
@section('title', 'Tambah Kategori Video')
...
<div class="space-y-6 pb-12">
    <div>
        ...
        <h1 class="text-2xl font-black text-slate-800 tracking-tight">Tambah Kategori Modul Baru</h1>
    </div>

    <form action="{{ route('admin.video-categories.store') }}" method="POST" enctype="multipart/form-data" class="bg-white p-6 md:p-8 rounded-3xl border border-slate-200 shadow-sm space-y-6 max-w-3xl">
        @csrf

        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
                <label class="block text-xs font-bold text-slate-600 mb-2">Badge Modul</label>
                <input type="text" name="badge" value="{{ old('badge', 'Modul 01') }}" required placeholder="Contoh: Modul 01"
            ...
            <div>
                ...
                <input type="text" name="title" value="{{ old('title') }}" required placeholder="Contoh: Fisika & Energi Terbarukan"
                       class="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 text-xs font-bold text-slate-800 outline-none focus:border-emerald-500">
            </div>
        </div>
        ...
        <div>
            ...
            <textarea name="description" rows="3" required placeholder="Tuliskan gambaran ringkas modul ini..."
                      class="w-full bg-slate-50 border border-slate-200 rounded-xl p-4 text-xs font-medium text-slate-800 outline-none focus:border-emerald-500 resize-none">{{ old('description') }}</textarea>
        </div>

        <div>
            <label class="block text-xs font-bold text-slate-600 mb-2">Upload Cover Thumbnail Image</label>
