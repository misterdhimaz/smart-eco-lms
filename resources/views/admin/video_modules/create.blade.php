...
<head>
    ...
    <title>Tambah Modul Video | SMART-ECO Admin</title>
...
<body class="bg-slate-50 text-slate-900 antialiased font-sans h-[100dvh] flex overflow-hidden" x-data="{ sidebarOpen: false }">
    ...
    <div class="flex-1 flex flex-col min-w-0 h-full overflow-hidden relative z-10 w-full bg-slate-100">
        ...
        <header class="h-14 md:h-[76px] bg-white border-b-2 border-slate-200 flex items-center justify-between px-3 md:px-8 z-30 shrink-0 shadow-sm">
            <div class="flex items-center gap-2 md:gap-4 truncate w-full md:w-auto">
                ...
                <div class="truncate">
                    <h1 class="text-sm md:text-xl lg:text-2xl font-black text-slate-900 tracking-tight leading-none truncate">Buat <span class="text-emerald-600">Modul Video</span></h1>
                </div>
            </div>
        </header>

        <!-- FORM WRAPPER (FLEX-COL AGAR FOOTER STICKY DI BAWAH) -->
        <form action="{{ route('admin.video-modules.store') }}" method="POST" enctype="multipart/form-data" class="flex-1 flex flex-col h-full overflow-hidden w-full" x-data="videoForm()">
            @csrf
                    ...
                    <div class="bg-white rounded-2xl md:rounded-[2rem] border-2 border-slate-200 shadow-sm p-4 md:p-8 w-full border-t-4 border-t-emerald-500">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-6 mb-4 md:mb-6">
                            <div>
                                ...
                                <input type="text" name="badge" value="{{ old('badge', 'Video Pembelajaran') }}" required placeholder="Contoh: Modul 01" class="w-full bg-slate-50 border-2 border-slate-200 rounded-xl px-4 py-3 text-xs md:text-sm font-bold text-slate-800 outline-none focus:border-emerald-600 focus:bg-white transition-colors shadow-sm">
                            </div>
                            <div>
                                <label class="block text-[10px] md:text-xs font-black text-slate-600 uppercase tracking-widest mb-1.5">Judul Utama</label>
                                <input type="text" name="title" value="{{ old('title') }}" required placeholder="Contoh: Dasar Perubahan Iklim" class="w-full bg-slate-50 border-2 border-slate-200 rounded-xl px-4 py-3 text-xs md:text-sm font-bold text-slate-800 outline-none focus:border-emerald-600 focus:bg-white transition-colors shadow-sm">
                            </div>
                        </div>
                        ...
                        <div class="mb-4 md:mb-6">
                            ...
                            <textarea name="description" rows="3" required placeholder="Tuliskan gambaran ringkas mengenai modul ini..." class="w-full bg-slate-50 border-2 border-slate-200 rounded-xl px-4 py-3 text-xs md:text-sm font-bold text-slate-800 outline-none focus:border-emerald-600 focus:bg-white resize-none transition-colors shadow-sm">{{ old('description') }}</textarea>
                        </div>

                        <div>
                            <label class="block text-[10px] md:text-xs font-black text-slate-600 uppercase tracking-widest mb-1.5">Cover Thumbnail Gambar</label>
                            <input type="file" name="cover_image" accept="image/*" required class="w-full text-xs font-bold text-slate-500 file:mr-3 file:py-2 file:px-4 file:rounded-lg file:border-0 file:bg-slate-800 file:text-white cursor-pointer border-2 border-dashed border-slate-300 rounded-xl p-2 bg-slate-50 hover:bg-slate-100 transition-colors">
                        </div>
                    </div>
                    ...
                    <div class="bg-white rounded-2xl md:rounded-[2rem] border-2 border-slate-200 shadow-sm p-4 md:p-8 w-full border-t-4 border-t-blue-600">
                        <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 border-b-2 border-slate-100 pb-4 mb-5">
                            <div>
                                ...
                                <p class="text-[9px] md:text-[10px] font-bold text-slate-400 mt-1">Dukung input dari Link YouTube dan file Upload MP4.</p>
                            </div>
                        ...
                        <div class="space-y-4 md:space-y-6">
                            <template x-for="(video, index) in videos" :key="video.id">
                                <div class="p-4 md:p-6 bg-slate-50 border-2 border-slate-200 rounded-xl md:rounded-2xl relative shadow-sm">

                                    <div class="flex items-center justify-between border-b-2 border-slate-200 pb-3 mb-4">
                                        <span class="text-[10px] md:text-xs font-black text-slate-700 uppercase tracking-wider flex items-center gap-2">
                                    ...
                                    <div class="p-4 md:p-5 rounded-xl border-2 transition-colors mb-4 md:mb-5" :class="video.type === 'youtube' ? 'bg-white border-rose-200' : 'bg-white border-indigo-200'">
                                        <div x-show="video.type === 'youtube'">
                                            ...
                                            <input type="text" :name="`videos[${index}][youtube_url]`" x-model="video.youtube_url" @input="detectYoutubeDetails(index)" placeholder="https://www.youtube.com/watch?v=..." class="w-full bg-slate-50 border-2 border-slate-200 rounded-xl px-4 py-3 text-xs md:text-sm font-bold text-slate-800 outline-none focus:border-rose-500 transition-colors shadow-inner">
                                            <p class="text-[9px] font-bold text-slate-400 mt-2">*Sistem akan mencoba mengisi durasi secara otomatis dari link.</p>
                                        </div>
                                        <div x-show="video.type === 'upload'" style="display: none;">
                                            <label class="block text-[9px] md:text-[10px] font-black text-indigo-600 uppercase tracking-widest mb-1.5">Unggah Berkas (MP4)</label>
                                            <input type="file" :name="`videos[${index}][video_file]`" accept="video/mp4,video/webm" @change="detectVideoDuration($event, index)" class="w-full text-xs font-bold text-slate-500 file:mr-3 file:py-2 file:px-4 file:rounded-lg file:border-0 file:bg-indigo-600 file:text-white hover:file:bg-indigo-700 cursor-pointer border-2 border-dashed border-slate-300 rounded-xl p-2 bg-slate-50 transition-colors">
                                        </div>
                                    </div>

                                    <div class="grid grid-cols-1 md:grid-cols-3 gap-4 md:gap-5 border-t-2 border-slate-200 pt-4 md:pt-5">
                                        <div>
                                            <label class="block text-[9px] md:text-[10px] font-black text-slate-500 uppercase tracking-widest mb-1.5">Durasi Otomatis/Manual</label>
                                            <input type="text" :name="`videos[${index}][duration]`" x-model="video.duration" required placeholder="00:00" class="w-full bg-white border-2 border-slate-200 rounded-xl px-4 py-3 text-xs md:text-sm font-mono font-black text-slate-800 outline-none focus:border-blue-500 transition-colors shadow-sm">
                                        </div>
                                        <div class="md:col-span-2">
                                            <label class="block text-[9px] md:text-[10px] font-black text-slate-500 uppercase tracking-widest mb-1.5">Deskripsi Ringkas Materi</label>
                                            <input type="text" :name="`videos[${index}][description]`" x-model="video.description" placeholder="Penjelasan singkat materi di video ini..." class="w-full bg-white border-2 border-slate-200 rounded-xl px-4 py-3 text-xs md:text-sm font-bold text-slate-800 outline-none focus:border-blue-500 transition-colors shadow-sm">
                                        </div>
                                    </div>
            ...
            <div class="bg-white/95 backdrop-blur-xl border-t-2 border-slate-200 p-4 shrink-0 z-20 shadow-[0_-5px_15px_rgba(0,0,0,0.05)] w-full">
                <div class="max-w-[1200px] mx-auto flex items-center justify-between gap-4">
                    ...
                    <div class="flex items-center justify-between sm:justify-end gap-2 md:gap-3 w-full sm:w-auto">
                        ...
                        <button type="submit" class="flex-1 sm:flex-none px-4 md:px-8 py-3.5 bg-emerald-600 hover:bg-emerald-700 text-white font-black rounded-xl text-[10px] md:text-xs uppercase tracking-widest transition-all shadow-md active:scale-95 flex items-center justify-center gap-2">
    ...
    <script>
        function videoForm() {
            return {
                videos: [
                    { id: Date.now(), title: '', type: 'youtube', youtube_url: '', duration: '00:00', description: '' }
                ],
                addVideo() { this.videos.push({ id: Date.now() + Math.random(), title: '', type: 'youtube', youtube_url: '', duration: '00:00', description: '' }); },
